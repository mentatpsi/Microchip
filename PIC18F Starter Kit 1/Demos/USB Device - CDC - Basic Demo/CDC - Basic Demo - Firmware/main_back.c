...
{   
	...
	if (executionTick == 200)
	{
		executionTick = 0;

				buffer[0] = BMA150_ReadByte(BMA150_ACC_X_LSB);
				buffer[1] = BMA150_ReadByte(BMA150_ACC_X_MSB);

				buffer[2] = BMA150_ReadByte(BMA150_ACC_Y_LSB);
				buffer[3] = BMA150_ReadByte(BMA150_ACC_Y_MSB);

				buffer[4] = BMA150_ReadByte(BMA150_ACC_Z_LSB);
				buffer[5] = BMA150_ReadByte(BMA150_ACC_Z_MSB);

				buffer[6] = BMA150_ReadByte(BMA150_TEMP);

				buffer[7] = 0x0D;
				buffer[8] = 0x0A;
				putUSBUSART(buffer, 9);
 	}   
		executionTick++;
	CDCTxService();
	return;
	...
	if (executionTick == 200)
	{
		...
		switch(commandIndex)
		{
			case 0:
				...
				sprintf(buffer, "X acc: %02d\n", 4*((signed char)reg_MSB.val));
				putsUSBUSART(buffer);
				commandIndex ++;
				break; 
