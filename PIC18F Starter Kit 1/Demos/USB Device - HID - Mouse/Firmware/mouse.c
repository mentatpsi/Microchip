...
/** VARIABLES ******************************************************/
typedef enum _SCROLL_WHEEL_STATES
{
    NO_TOUCH = 0,
    STARTED_UP_NOW_UP,
    STARTED_UP_NOW_MIDDLE,
    STARTED_UP_NOW_DOWN,
    STARTED_DOWN_NOW_DOWN,
    STARTED_DOWN_NOW_MIDDLE,
    STARTED_DOWN_NOW_UP,
    STARTED_MIDDLE
}SCROLL_WHEEL_STATES;
...
SCROLL_WHEEL_STATES scrollBarState = NO_TOUCH;
unsigned char scrollCounter = 0;
#define SCROLL_CONSTANT 100

char executionTick = 0;
...
void DemoIntroduction(void)
{
    switch(DemoIntroState)
    {
        case 0:
            ...
            oledPutROMString((ROM_STRING)"   Mouse Demo v1.0   ",1,0);
        ...
        case 2:
            ...
            oledPutROMString((ROM_STRING)"  a USB mouse.       ",2,0);
        ...
        case 4:
            ...
            oledPutROMString((ROM_STRING)"  move the cursor.   ",1,0);
            oledPutROMString((ROM_STRING)"Hold the board flat  ",2,0);
            oledPutROMString((ROM_STRING)"  relative to the    ",3,0);
            oledPutROMString((ROM_STRING)"  ground for no      ",4,0);
            oledPutROMString((ROM_STRING)"  movement.          ",5,0);
            oledPutROMString((ROM_STRING)"Press Menu to proceed",6,0);
            DemoIntroState = 5;
            break;
        ...
        case 6:
            oledPutROMString((ROM_STRING)"Press the 'L' button ",0,0);
            oledPutROMString((ROM_STRING)"  to left click, and ",1,0);
            oledPutROMString((ROM_STRING)"  the 'R' button to  ",2,0);
            oledPutROMString((ROM_STRING)"  right click.       ",3,0);
            oledPutROMString((ROM_STRING)"                     ",4,0);
            oledPutROMString((ROM_STRING)"                     ",5,0);
            oledPutROMString((ROM_STRING)"Press Menu to proceed",6,0);
            DemoIntroState++;
...
{   
	...
	if (executionTick == 100)
	{
		/* It is time to read acceleration data*/
		
		/*Reset the executionTick variable */
		executionTick = 0;
		...
		else
		{
			...
			/* Because of tangent properties, we can extrapolate for negative numbers */
			isNegativ = 0;
			if (accY < 0)
			{
				accY = - accY;
		...
		/* In this section we read the capacitive buttons and the slider */
