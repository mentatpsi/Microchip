...
********************************************************************
 ...
 Change History:
  ...
  1.0   27/02/2009   Initial release
...
//The direction that the Joystick will move in
ROM unsigned int arctan_table[]={6,11,17,23,28,34,40,47,53,60,67,74,82,90,98,107,117,128,140,153,167,183,201,222,246,274,309,352,406,478,577,726,972,1462,2929};

// The Joystick input controls
...
typedef union _PS3_INTPUT_CONTROLS_TYPEDEF
{
    struct
    {
        struct
...
void DemoIntroduction(void)
{
    switch(DemoIntroState)
    {
        case 0:
            ...
            oledPutROMString((ROM_STRING)" Joystick Demo v1.0  ",1,0);
        ...
        case 2:
            ...
            oledPutROMString((ROM_STRING)"  a USB joystick.    ",2,0);
        ...
        case 4:
            oledPutROMString((ROM_STRING)"Tilt the board to    ",0,0);
            oledPutROMString((ROM_STRING)"  move the joystick  ",1,0);
            oledPutROMString((ROM_STRING)"  on the X or Y axes;",2,0);
            oledPutROMString((ROM_STRING)"  use the scroll bar ",3,0);
            oledPutROMString((ROM_STRING)"  to move it on the  ",4,0);
            oledPutROMString((ROM_STRING)"  Z axes.            ",5,0);
            oledPutROMString((ROM_STRING)"Press Menu to proceed",6,0);
            DemoIntroState = 5;
            break;
        ...
        case 6:
            oledPutROMString((ROM_STRING)"Use the potentiometer",0,0);
            oledPutROMString((ROM_STRING)"  to rotate around   ",1,0);
            oledPutROMString((ROM_STRING)"  the Z axes, touch  ",2,0);
            oledPutROMString((ROM_STRING)"  the L/R capacitive ",3,0);
            oledPutROMString((ROM_STRING)"  pads to control the",4,0);
            oledPutROMString((ROM_STRING)"  1st/2nd button.    ",5,0);
            oledPutROMString((ROM_STRING)"Press Menu to proceed",6,0);
            DemoIntroState++;
            break;
        case 7:
 ...
 *
 * Overview:        The routine has following functionalities
 *					1) Read the Acceleration from BMA150 and updates the X-Y axis of the Joystick 
 *					2) Read the mTtouch switches and use it for Joystick switches.
					3) Read mTouch slider and use it for the Z-axis. 
 *					4) Read the potentiometer values and use it for Z-rotation 
 *					
...
void Joystick(void)
{
	signed char accX, accY, accZ; // Acceleration in 3 directions. 
	signed int tiltX, tiltY;
	WORD SliderVoltage[2] = {0,0}; //for mtouch channel analog voltages  
	signed int Slider_Difference=0; 
	WORD_VAL w; 
	unsigned char i;
	unsigned char isNegativ;
	unsigned char counter ;  
	//signed int difference =0;
    if((USBDeviceState < CONFIGURED_STATE)||(USBSuspendControl==1)) return;

	// Analog Stick //
	// The below portion of the code for updating the X-Y axis of the joystick using BMA150 (accelerometer) 

	/* Read LSB and MSB, but only MSB is used */
	accX = (signed char)BMA150_ReadByte(BMA150_ACC_X_LSB);
	accX = (signed char)BMA150_ReadByte(BMA150_ACC_X_MSB);
	
	/* Read LSB and MSB, but only MSB is used */
	accY = (signed char)BMA150_ReadByte(BMA150_ACC_Y_LSB);
	accY = (signed char)BMA150_ReadByte(BMA150_ACC_Y_MSB);

	/* Read LSB and MSB, but only MSB is used */
	accZ = (signed char)BMA150_ReadByte(BMA150_ACC_Z_LSB);
	accZ = (signed char)BMA150_ReadByte(BMA150_ACC_Z_MSB);

	if ((accX == 0)&&(accY == 0)&&(accZ == 0))
	{
        /* No acceleration detected on Z . To avoid division by zero, we don't compute mouse orientation */
	...
	else if (accZ != 0)
	{
		...
		/* Because of tangent properties, we can work only with positive numbers */
		isNegativ = 0;
		if (accX < 0)
		{
			accX = - accX;
		...
		for (i = 0; i < 35; i++)
		{
			if (tiltX < arctan_table[i])
			{
				if (isNegativ == 1)
	            {
					/* Setting the sensitivity */
		            buffer[2] = 128-(i*7)/2;           // Y-Vector
				}
	            else
				{
					/* Setting the sensitivity */
		...
		for (i = 0; i < 35; i++)
		{
			if (tiltY < arctan_table[i])
			{
				if (isNegativ == 1)
	            {
					/* Setting the sensitivity */
		            buffer[1] = 128-(i*7)/2;           // X-Vector
				}
	            else
				{
					/* Setting the sensitivity */
	...
	PS3_input.members.analog_stick.X = buffer[1]; //prepare to send the x-axis to joystick 
	PS3_input.members.analog_stick.Y = buffer[2]; //prepare to send the y-axis to joystick
		
	
	
	// Z-Axis (using mtouch slider)
	// The below portion of the code for Z axis of the joystick using mtouch 2 Channel Slider
	for (counter=0; counter<=16; counter++)  //16 samples of the slider readings
	{
 		SliderVoltage[0] += mTouchReadButton(MTOUCH_SLIDER_TOP);
		SliderVoltage[1] += mTouchReadButton(MTOUCH_SLIDER_BOTTOM);
	}
	SliderVoltage[0] = SliderVoltage[0]/16;  // average of the samples. 
	SliderVoltage[1] = SliderVoltage[1]/16; // average of the samples. 
	Slider_Difference = SliderVoltage[1] - SliderVoltage[0]; // differnce of the two channels. 
	Slider_Difference = Slider_Difference*2/3; //scaling the diffrence for the joystick Z-axis.
	if (Slider_Difference <= -127) //setting minimum 
		Slider_Difference = -127; 
	else if (Slider_Difference >= 127) //setting maximum
		Slider_Difference = 127;
	PS3_input.members.analog_stick.Z = 127 + Slider_Difference; // add with 127 becuase the Z-axis is- 
										//	should be at middle when not touched or touched in the middile.

	
	//Z-Rotation (using potentiometer). 
	// The below portion of the code for Z axis of the joystick using mtouch 2 Channel Slider
	w= ReadPOT();
	PS3_input.members.analog_stick.Rz = (w.Val)/4; 


	//Right Switch (using mtouch switch)
	SliderVoltage[0] = mTouchReadButton(MTOUCH_SWITCH_R);
	if (SliderVoltage[0] <= 600)
		PS3_input.members.buttons.x = 1;
	else 
		PS3_input.members.buttons.x = 0;


	//Left Swicth (using mtouch switch)
	SliderVoltage[0] = mTouchReadButton(MTOUCH_SWITCH_L);
	if (SliderVoltage[0] <= 600)
		PS3_input.members.buttons.square = 1;
	else 
		PS3_input.members.buttons.square = 0;	

	
	PS3_input.members.buttons.o = 0;
    PS3_input.members.buttons.triangle = 0;
    PS3_input.members.buttons.L1 = 0; 
	PS3_input.members.buttons.R1 = 0;   
    PS3_input.members.buttons.L2 = 0;
    PS3_input.members.buttons.R2 = 0;
    PS3_input.members.buttons.select = 0;
    PS3_input.members.buttons.start = 0;
	PS3_input.members.buttons.right_stick = 1;
	PS3_input.members.buttons.left_stick = 1;

    PS3_input.members.buttons.home = 0;
    PS3_input.members.hat_switch.hat_switch = 0;

    //Send the 8 byte packet over USB to the host.
    lastTransmission = HIDTxPacket(HID_EP, (BYTE*)&PS3_input, sizeof(PS3_input));
