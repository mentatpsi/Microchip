 ...
 * Brian Tompson        9/21/12     Added timing adjustments to support
 *                                  for PIC18F87J94 PIM on PIC18 
 *                                  Explorer demo board
 ********************************************************************/
...
#define SAMSUNG_S6A0032		// This LCD driver chip has a different means of entering 4-bit mode.  
    ...
    #include "MSPI.h"
    #define CLOCK_ADJUST    GetInstructionClock() / 670000
    // Some PIC devices have Fosc speeds at fast at 64 MHz
    // CLOCK_ADJUST is used to adjust the execution based timing
    // for these faster devices
    ...
    {
        BYTE   clockMultiplier = CLOCK_ADJUST;
        ...
        do
        {
            ...
            Nop();
            Nop();
        } while( clockMultiplier-- );

        LCDWritePortA(0x40);
        clockMultiplier = CLOCK_ADJUST;
        do
        {
            Nop();
            Nop();
