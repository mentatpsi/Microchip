...
********************************************************************
 ...
 Change History:
  ...
  2.9g  Adding modified to support multiple LCD displays
********************************************************************/
...
#elif defined(PIC18F97J94_FAMILY)
        #pragma config STVREN   = ON      	// Stack overflow reset
        #pragma config XINST    = OFF   	// Extended instruction set
...
{
    USB_Rx_READ_POT         = 0x37,
    USB_Rx_SET_RTCC         = 0x60,
    USB_Rx_GET_RTCC         = 0x61,
    USB_Rx_LCD_SPECIALS     = 0x70,
    USB_Rx_LCD_SET_BANNER   = 0x71,
    USB_Rx_LCD_DISPLAY_RTCC = 0x72,
    USB_Rx_LED_TOGGLE       = 0x80,
    USB_Rx_GET_BUTTON       = 0x81,

} HOST_CMD_TYPE;


typedef enum LCD_DISPLAY_STATE
{
    LCD_DISPLAY_BANNER0     = 0,
    LCD_DISPLAY_BANNER1     = 1,
    LCD_DISPLAY_BANNER2     = 2,
    LCD_DISPLAY_DATE        = 3,
    LCD_DISPLAY_TIME        = 4,
    LCD_DISPLAY_SPLASH      = 5,

} LCD_DISPLAY_STATE;


typedef enum DISPLAY_DELAY_TYPE
{                                       // TMR0 ticks about every 131 mS
    DISPLAY_TIME_DELAY      = 0xFE,     // ~33 seconds = 131 mS * 0xFE
    DISPLAY_DATE_DELAY      = 0x40,     // ~ 8 seconds = 131 mS * 0x40
    DISPLAY_SPLASH_DELAY    = 0x20,     // ~ 4 seconds = 131 mS * 0x20
...
/* Global value for RTCC time*/
DTVALUE RTCC_DTV;
// Global LCD variables
LCD_DISPLAY_STATE LCDState      = 0x00;
volatile BOOL PeriodicRefresh   = FALSE;
BYTE LCDRefreshCount            = 0x00;
BOOL LCDDisplayRTCC             = TRUE;

#if defined(PIC18F97J94_PIM)
    #define MCU_NAME    "PIC18F97J94 "
#elif defined(PIC18F87J94_PIM)
    #define MCU_NAME    "PIC18F87J94 "
#else // MCU Type unknown
    #error "device unknown: Check device configuration"
#endif // PIC18F97J94

BANRSTR Banr[LCDMAXBANR];
const ROM BANRSTR BanrPGM[LCDMAXBANR] = {
	{"      Presenting the " MCU_NAME,              0, 0, FALSE},   // 0
	{"      With USB,LCD,EMB,VBAT and More!!! ",    0, 0, FALSE},   // 1
	{"      Your Text Here ",                       0, 0, FALSE},   // 2
	};
...
{   
	...
	if(a == 0x56)
	{
		mInitAllLEDs();         // TRISHbits.TRISH2 = 0;
		                        // TRISHbits.TRISH3 = 0;
		mLED_1_On();            // LATHbits.LATH2 = 1;
		mLED_2_On();            // LATHbits.LATH3 = 1;
		Reset();
		while(1);
	}	
    ...
    while(1)
    {
        ...
        {  
        	...
        	Get_RTCC_Value(&RTCC_DTV);
	        
	        //*************************************//
	        // LCD Banner Update States            //
	        //*************************************//
	        if(LCDState < LCD_DISPLAY_DATE)
	        {
		        if(Banr[LCDState].ScrlCmplt == TRUE)
				{
					Banr[LCDState].ScrlCmplt = FALSE;
					LCDState++;
					if((LCDDisplayRTCC == FALSE) && (LCDState > 2))
	        		{
		        		LCDState = LCD_DISPLAY_BANNER0;	
		        	}
				}
                else
                {
                    UpdateBanner(&Banr[LCDState]);
                }
	        }
	        
	        //*************************************//
	        // LCD Date Update State               //
	        //*************************************//
	        if(LCDState == LCD_DISPLAY_DATE)
	        {

                DisplayDate( &RTCC_DTV );
		        LCDRefreshCount++;
		        if(LCDRefreshCount > DISPLAY_DATE_DELAY)
		        {
			        LCDState++;
			        LCDRefreshCount = 0;
			    } 
		    ...
		    //*************************************//
	        // LCD Time Update State               //
	        //*************************************//
		    if(LCDState == LCD_DISPLAY_TIME)
		    {
		        
                DisplayTime( &RTCC_DTV );
			    LCDRefreshCount++;
			    if(LCDRefreshCount > DISPLAY_TIME_DELAY)
		        {
			        LCDState = LCD_DISPLAY_BANNER0;
			        LCDRefreshCount = 0;
			    } 
			...
			//*************************************//
	        // LCD Initial Splash Screen           //
	        //*************************************//
			if(LCDState == LCD_DISPLAY_SPLASH)
			{

                DisplaySplash();
				LCDRefreshCount++;
			    if(LCDRefreshCount > DISPLAY_SPLASH_DELAY)
		        {
			        LCDState = LCD_DISPLAY_BANNER0;
			        LCDRefreshCount = 0;
			    } 
...
static void InitializeSystem(void)
{
        ...
        ANCON3 = 0xFF;
            ...
            while(OSCCON2bits.LOCK == 0);   //Make sure PLL is locked/frequency is compatible
                ...
                SSP1STAT    = 0xC0;     // CKE, SMP
                SSP1CON1    = 0x22;     // SSMP1 = 2, SSPEN
            #else
                SPI_SDO     = 0;
                SPI_SCK     = 0;
            #endif // defined(HARDWARE_SPI)
        #endif // defined(PIC18_EXPLORER)
...
void UserInit(void)
{
    ...
    // Must Copy the Default strings from flash into RAM for LCD Banner update
	strcpypgm2ram(&Banr[0].String[0],&BanrPGM[0].String[0]);
	Banr[0].CharStrtPt = 0;
	Banr[0].ColStrtPt = 0;
	Banr[0].ScrlCmplt = FALSE;
	strcpypgm2ram(&Banr[1].String[0],&BanrPGM[1].String[0]);
	Banr[1].CharStrtPt = 0;
	Banr[1].ColStrtPt = 0;
	Banr[1].ScrlCmplt = FALSE;
	strcpypgm2ram(&Banr[2].String[0],&BanrPGM[2].String[0]);
	Banr[2].CharStrtPt = 0;
	Banr[2].ColStrtPt = 0;
	Banr[2].ScrlCmplt = FALSE;
	
	// Initialize the LCD Refresh Variables
    LCDState = LCD_DISPLAY_SPLASH;  // State 5 displays an initial splash screen
    LCDRefreshCount = 0;    // Counts of time in a single 
    LCDDisplayRTCC = TRUE;  // Sets flag for displaying the RTCC value on the LCD or not

    LCDInit();              // Initializes the LCD driver

	
    PeriodicRefresh = FALSE; // refresh flag set by timer 0 to control banner scroll rate
    
    // Initialize Timer 0 for LCD Banner scroll refresh functions
    T0CON               = 0;        // clear T0CON
    T0CONbits.T0PS      = 4;        // Prescale 1:32
    T0CONbits.PSA       = 0;        // TMR0 input comes from prescalar output
    T0CONbits.T0CS      = 1;        // clock source (Fosc/4)
    T0CONbits.T08BIT    = 0;        // TMR0 is a 16-bit timer
    TMR0H               = 0x00;     // initial value 0x0000
    TMR0L               = 0x00;
                                    // TMR0 period = 1/(Fosc/4/32/0xFFFF)
                                    // Fosc = 64MHz
                                    // TMR0 period = ~131 mS

    T0CONbits.TMR0ON    = 1;        // turnon TMR0
    
    // Enable low priority Timer 0 interrupt to allow LCD refreshes.
    ...
    // Is this a true POR?
    // If not Vbat is retaining RTCC we shouldn't update its value
    if(RCON3bits.VBPOR)
    {
	    //Initalize and start the RTCC
	    ...
	    RTCC_DTV.DOW    = 0x00;
	    RTCC_DTV.Hour   = 0x00;
	    RTCC_DTV.Minute = 0x00;
	    RTCC_DTV.Second = 0x00;
	    RTCC_Init(&RTCC_DTV, RTCCSOSC);
	    /*WARNING! - if using the PIC18F97J94 PIM hardware RTCC could be 
	    off due to segment pins located near SOSCO or SOSCI. User 
	    applications should avoid placement of any toggling pins near 
	    SOSCO or SOSCI in final designs to achieve most accurate RTCC 
	    performance.*/
	}
    
    // Update reset flags so we can detect a true POR
    RCON3bits.VBPOR = 0;
    RCON3bits.VBAT = 0;
}//end UserInit
...
{   
    ...
    {   
        USB_Command         = ReceivedDataBuffer[0];
        ToSendDataBuffer[0] = USB_Command;      // prepare to echo back to the host the command we are fulfilling in the first byte.

        switch(USB_Command)
        {
	        case USB_Rx_READ_POT:	//Read POT command.  Uses ADC to measure an analog voltage on one of the ANxx I/O pins, and returns the result to the host
            ...
            case USB_Rx_SET_RTCC:  //Set RTCC Value from PC GUI to PIC

		        RTCC_DTV.Year = ReceivedDataBuffer[1];
		        RTCC_DTV.Month = ReceivedDataBuffer[2];
		        RTCC_DTV.Day = ReceivedDataBuffer[3];
		        RTCC_DTV.DOW = ReceivedDataBuffer[4];
		        RTCC_DTV.Hour = ReceivedDataBuffer[5];
		        RTCC_DTV.Minute = ReceivedDataBuffer[6];
		        RTCC_DTV.Second = ReceivedDataBuffer[7];
		        Set_RTCC_Value(&RTCC_DTV);
		        break;

            case USB_Rx_GET_RTCC:  //Get RTCC Value from PIC to the PC GUI

		        //Check to make sure the endpoint/buffer is free before we modify the contents
                if(!HIDTxHandleBusy(USBInHandle))
                {
					ToSendDataBuffer[1] = RTCC_DTV.Year;   //
					ToSendDataBuffer[2] = RTCC_DTV.Month;  //
					ToSendDataBuffer[3] = RTCC_DTV.Day;    //
					ToSendDataBuffer[4] = RTCC_DTV.DOW;    //
					ToSendDataBuffer[5] = RTCC_DTV.Hour;   //
					ToSendDataBuffer[6] = RTCC_DTV.Minute; //
					ToSendDataBuffer[7] = RTCC_DTV.Second; //	
                    //Prepare the USB module to send the data packet to the host
                    USBInHandle = HIDTxPacket(HID_EP,(BYTE*)&ToSendDataBuffer[0],64);
                }  
	       		break; 

            case USB_Rx_LCD_SPECIALS:  //Toggle LCD Special Pixels

		        LCDToggleSpecialPixels();
                break;

            case USB_Rx_LCD_SET_BANNER:  //Set LCD Custom Banner from PC GUI to PIC

            	// If we're already in the middle of another scrolling banner
            	// clear the scroll count then switch to the users custom input.
            	if(LCDState < LCD_DISPLAY_BANNER2)
            	{
            		Banr[LCDState].CharStrtPt = 0;
            		Banr[LCDState].ColStrtPt = 0;
            		Banr[LCDState].ScrlCmplt = FALSE;
            	}
            	// Switch to displaying the users input state	
            	LCDState = LCD_DISPLAY_BANNER2;
            	
            	// Copy PC GUI input to local RAM string
		        for(i=0;(i<(BANRSTRMAX-SCRLOFFSET)&&ReceivedDataBuffer[i+1]);i++)
		        {
			        Banr[2].String[i+SCRLOFFSET] = ReceivedDataBuffer[i+1];
			    } 
			    // Null terminate string
			    for(i=i;i<(BANRSTRMAX-SCRLOFFSET);i++)
			    {
				    Banr[2].String[i+SCRLOFFSET] = 0x00;
				}
                break;

            case USB_Rx_LCD_DISPLAY_RTCC:  //Set Display RTCC on LCD

		        LCDDisplayRTCC = ReceivedDataBuffer[1];
                break;    


            case USB_Rx_LED_TOGGLE:  //Toggle LEDs
            ...
            case USB_Rx_GET_BUTTON:  //Get push button state
