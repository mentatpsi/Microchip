...
0x03, 0x20,                         /* dev-config-id | standard configuration - Thermometer*/
...
0x03, 0x20,                           /*  config-report-id = 800 (Thermometer)*/
...
0x00, 0x01,                           /*  obj-handle = 1 (.. 1st Measurement is temperature) */
...
0x00, 0x02, 0x4B, 0x5C,               /*  MDC_PART_SCADA, MDC_TEMP_BODY */
...
'M', 'i', 'c', 'r', 'o', 'c', 'h', 'i' ,'p', ' ',          
0x00, 0x0C,                      /* string length = 12 | "Thermometer "  */
...
0x20, 0x08,  /* CertifiedDeviceClassEntry = MDC_DEV_SPEC_PROFILE_TEMP - 4096 + v_i_TCode = 0x2008 
			 	where, MDC_DEV_SPEC_PROFILE_TEMP = 4104, v_i_TCode = 1 for USB Agent */   
...
0xFB, 0x38, //0x75, 0x20, /*Simple-Nu-Observed-Value = 37.0 (Degress C)*/
0x20, 0x11, 0x05, 0x07, /*Absolute-Time-Stamp = 2011-04-06T12:10:0000*/
0x14, 0x39, 0x00, 0x00,
};
...
void PhdThermoMeterUpdateMeasurements(void);
       ...
       if(dataSendingUnderProgress == FALSE)
       {
		   ...
		   else if(AppState == PHD_CONNECTED)
		   {
		       ...
		       {    
    	           ...
    	           PhdThermoMeterUpdateMeasurements();
		           PHDSendMeasuredData();
		       }
...
/******************************************************************************
 * Function:        void UpdateThermometerMeasurements(void)
 *
 * PreCondition:    None
 *
 ...
 * Side Effects:    None
 *
 * Overview:        This routine updates the measured Thermometer values in to corresponding 
 *                  APDU locations	
 *
 * Note:            
 *
...
void PhdThermoMeterUpdateMeasurements(void)
{
    /* User can update the Measured value from the Thermometer*/
    /* Please go through IEEE_STD_11073_10415 (Thermometer device spec)*/
    UINT8 Temperature;
    ...
    PhdAppBuffer[34]= 0x00;  	//Measured analog voltage LSB
    PhdAppBuffer[35]= Temperature;  	//Measured analog voltage MSB
...
/******************************************************************************
 * Function:        UINT8 GetTemperature(void)
 *
 * PreCondition:    None
 *
 * Input:           None
