...
/* Association Request. The agent sends the following message to the manager. This example assumes the agent is an Standard 
Glucose Meter. */
...
0x01, 0x90,                         /* dev-config-id | standard configuration - Pulse Oximeter(400)*/
...
0x00, 0x46,                 /* CHOICE.length = 68 */
0x00, 0x44,                 /* OCTET STRING.length = 66 */
0x00, 0x03,                 /*  invoke-id = 3 (start of DataApdu.MDER encoded.) */
0x01, 0x01,                 /*  CHOICE(Remote Operation Invoke | Confirmed Event Report) */                 
0x00, 0x3E,                 /*  CHOICE.length = 60 */
0x00, 0x00,                 /*  obj-handle = 0 (MDS object) */
0xFF, 0xFF, 0xFF, 0xFF,     /*  event-time = 0xFFFFFFFF    */

0x0D, 0x1C,                 /*  event-type = MDC_NOTI_CONFIG */
0x00, 0x34,                 /*  event-info.length = 50 (start of ConfigReport) */ 
0x01, 0x90,                 /*  config-report-id = 400 (Pulse Oximeter)*/
0x00, 0x01,                 /*  config-obj-list.count = 1 Measurement objects will be announced */
0x00, 0x2E,                 /*  config-obj-list.length = 44 */

0x00, 0x06,                 /*  attribute-id = MDC_ATTR_ID_HANDLE */// (8 Bytes)
0x00, 0x01,                 /*  attribute-value.length = 2        */
0x00, 0x05,                 /*                                    */
0x00, 0x26, 
...
0x40, 0x40,                 /*  intermittent, stored data, upd & msmt aperiodic, agent init, measured */
                                          
0x09, 0x96,                 /*  attribute-id = MDC_ATTR_UNIT_CODE */ //(6 Bytes) 
0x00, 0x02,                 /*  attribute-value.length = 2 */
0x02, 0x20,                 /*  MDC_DIM_PERCENT */ 

0x0A, 0x73,                 /* attribute-id = MDC_ATTR_METRIC_STRUCTURE_SMALL */ //(6 Bytes)
0x00, 0x02,                 /* attribute-value.length = 2 */
0x00, 0x01, 

0x0A, 0x55,                 /*  attribute-id = MDC_ATTR_ATTRIBUTE_VAL_MAP  */ //(12 Bytes) 
0x00, 0x08,                 /*  attribute-value.length = 12     */
0x00, 0x01,                 /*  AttrValMap.count = 2         */
0x00, 0x04,                 /*  AttrValMap.length = 8          */
...
'M', 'i', 'c', 'r', 'o', 'c', 'h', 'i' ,'p', ' ',          
0x00, 0x0C,                      /* string length = 12 | "GlucoseMeter"  */
'P', 'u', 'l', 's', 'e', ' ', 'O', 'x', 'i', 'm', 't', 'r', 
...
0x20, 0x11,  /* CertifiedDeviceClassEntry = MDC_DEV_SPEC_PROFILE_GLUCOSE - 4096 + v_i_TCode * 8192 = 0x2011 
                where, MDC_DEV_SPEC_PROFILE_GLUCOSE = 4113, v_i_TCode = 1 for USB Agent */   
...
0x00, 0x26, /*OCTET STRING.length = 38*/
0x00, 0x08, /*invoke-id = 8*/
0x01, 0x01, /*CHOICE(Remote Operation Invoke | Confirmed Event Report)*/
0x00, 0x20, /*CHOICE.length = 32*/
0x00, 0x00, /*obj-handle = 0 (MDS object)*/
...
0xF3, 0x52, /*Simple-Nu-Observed-Value = 85.0 (mmol/L)*/
0x20, 0x08, 0x09, 0x19, /*Absolute-Time-Stamp = 2011-04-06T12:10:0000*/
0x17, 0x30, 0x00, 0x00
};
...
BYTE sw2ButtonPressCounter; 
 
BYTE phdAppState, phdMeasurementTransferSate; 
BOOL allowButtonSw2Action ,allowButtonSw3Action;
UINT16_VAL scanReportNo;

/** P R I V A T E  P R O T O T Y P E S ***************************************/
void PhdPulseOximeterUpdateMeasurements(void);
...
{
    phdAppState = PHD_DISCONNECTED;
    phdMeasurementTransferSate = PHD_MEASUREMENT_SENT; 
    allowButtonSw2Action = FALSE;
    allowButtonSw3Action = FALSE;
...
// Some of the Microchip USB demo boards have only one pushbutton. In those boards with only one pushbutton, 
// if the pushbutton is pressed for the first time then the PHD Agent sends an Association request to the  Manager. 
// If the push button is pressed for the second, third or fourth time the Agent sends some measured data 
// to the Manager. If the Pushbutton is pressed  for the Fifth time the Agent disconnects from the Manager and resets the push button counter.      
    if (buttonPressedsw2)
    {
       if(allowButtonSw2Action == TRUE)
       {
           if(phdAppState == PHD_DISCONNECTED)
           {
               PHDConnect();
               phdAppState = PHD_CONNECTING; 
           }
           else if (phdAppState == PHD_CONNECTED)
           {    
               // if the push button is pressed for the Fifth time then the agent sends Measured Data.        
               phdAppState = PHD_DISCONNECTING;
               PHDDisConnect();
               phdMeasurementTransferSate = PHD_MEASUREMENT_SENT;
           }
           allowButtonSw2Action=FALSE; //disallow the sw2 pushbutton action. Wait for the user to release the pushbutton 
       } 
    } 
    else
    {
        allowButtonSw2Action = TRUE; // User has released pushbutton. Now allow the Sw2 action. 
    }     
    if((phdAppState == PHD_CONNECTED) && (phdMeasurementTransferSate == PHD_MEASUREMENT_SENT))
    {
        PhdPulseOximeterUpdateMeasurements(); 
        PHDSendMeasuredData();
        phdMeasurementTransferSate = PHD_MEASUREMENT_SENDING; 
    }
        ...
        if(buttonPressedsw2)
        {
            if(allowButtonSw2Action == TRUE)
            {
                ...
                else if((phdAppState == PHD_CONNECTED)&& (phdMeasurementTransferSate == PHD_MEASUREMENT_SENT))
                {
                    phdAppState = PHD_DISCONNECTING;
                    PHDDisConnect();        
                }
        ...
        {
            allowButtonSw2Action = TRUE;
        }
        if(buttonPressedsw3)
        {
            if(allowButtonSw3Action == TRUE)
            {
                if(phdAppState == PHD_CONNECTED)
                {
                    PhdPulseOximeterUpdateMeasurements();
                    PHDSendMeasuredData();
                    phdMeasurementTransferSate = PHD_MEASUREMENT_SENDING; 
...
/******************************************************************************
 * Function:        void PhdPulseOximeterUpdateMeasurements(void)
 *
 * PreCondition:    None
 *
 ...
 * Side Effects:    None
 *
 * Overview:        This routine updates the measured Pulse Oximeter values in to corresponding 
 *                  APDU locations  
 *
 * Note:            
 *
...
void PhdPulseOximeterUpdateMeasurements(void)
{
    /* User can update the Measured value from the Glucose Meter*/
    /* Please go through IEEE_STD_11073_10417 (Glucometer device spec)*/
    UINT16_VAL oxygenSaturation; 
    ...
    oxygenSaturation.Val = GetOxygenSaturation();
    PhdAppBuffer[34]= oxygenSaturation.byte.HB;     //Measured analog voltage LSB
    PhdAppBuffer[35]= oxygenSaturation.byte.LB;     //Measured analog voltage MSB
    
    
    /************ Update Time Stamp **********************/ 
    #if defined PHD_USE_RTCC_FOR_TIME_STAMP
        RtccReadTimeDate(&TimeStamp);
        PhdAppBuffer[36] = 0x20;                    //Year 1
        PhdAppBuffer[37] = TimeStamp.f.year ;       //Year 2TimeStamp.f.year  
        PhdAppBuffer[38] = TimeStamp.f.mon;         // Month 
        PhdAppBuffer[39] = TimeStamp.f.mday;        // Day
        PhdAppBuffer[40] = TimeStamp.f.hour;        // Hour
        PhdAppBuffer[41] = TimeStamp.f.min;         // Minute
        PhdAppBuffer[42] = TimeStamp.f.sec;         // Seconds
        PhdAppBuffer[43] = 0x00;
    #else 
        /* Time Stamp  = 12:30:00, 20th July 2011*/ 
        PhdAppBuffer[36] = 0x20;           //Year 1
        PhdAppBuffer[37] = 0x11;           //Year 2
        PhdAppBuffer[38] = 0x11;           // Month 
        PhdAppBuffer[39] = 0x18;           // Day
        PhdAppBuffer[40] = 0x14;            // Hour
        PhdAppBuffer[41] = 0x39;           // Minute
        PhdAppBuffer[42] = 0x00;           // Seconds
        PhdAppBuffer[43] = 0x00;
    #endif     
}
/******************************************************************************
 * Function:        UINT8 GetOxygenSaturation(void)
 *
 * PreCondition:    None
 *
 * Input:           None
 ...
 * Side Effects:    None
 *
 * Overview:        This routine Gets the Oxygen Saturation Level simulated by Potentiometer.   
 *
 * Note:            
 *
 *****************************************************************************/
UINT16 GetOxygenSaturation(void)
{
  UINT16 oxygenSaturation;
  WORD_VAL w;  
