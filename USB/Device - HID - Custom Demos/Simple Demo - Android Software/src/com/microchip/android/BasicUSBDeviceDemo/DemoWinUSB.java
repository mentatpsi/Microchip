 ...
 */
public class DemoWinUSB extends Demo implements Runnable, DemoInterface {
	...
	private final static int WINUSB_DEMO_BUTTON_PRESSED = (int)0x00;
	 ...
	 */
	DemoWinUSB(Context context, UsbDevice device, Handler handler) {
	...
	public String getDeviceTitle() {
		if(device != null) {
			return "WinUSB Device Demo (VID = 0x" + Integer.toHexString(device.getVendorId()) + 
	...
	public void run() {	
		...
		while(true) {
			...
			if(result > 0) {
				...
				boolean currentButtonState = (getPushButtonStatusResults[1]==WINUSB_DEMO_BUTTON_PRESSED)?true:false;
