...
0x06, 0xA4,                         /* dev-config-id | standard configuration - Glucose Meter*/
...
0x00, 0x42,                 /* OCTET STRING.length = 66 */
...
0x00, 0x32,                 /*  event-info.length = 50 (start of ConfigReport) */ 
0x06, 0xA4,                 /*  config-report-id = 1700 (Glucose Meter)*/
...
0x00, 0x01,                 /*  obj-handle = 1 (.. 1st Measurement is blood glucose) */
...
0x00, 0x02, 0x71, 0xB8,     /*  MDC_PART_SCADA|MDC_CONC_GLU_CAPILLARY_WHOLEBLOOD */
...
void PhdGlucoseMeterUpdateMeasurements(void);
       ...
       if(dataSendingUnderProgress == FALSE)
       {
		   ...
		   else if(AppState == PHD_CONNECTED)
		   {
		       if (sw2ButtonPressCounter == 2)
		       {
				   ...
				   PhdGlucoseMeterUpdateMeasurements(); 
		           PHDSendMeasuredData();
		       }
...
/******************************************************************************
 * Function:        void PhdGlucoseMeterUpdateMeasurements(void)
 *
 * PreCondition:    None
 *
 ...
 * Side Effects:    None
 *
 * Overview:        This routine updates the measured Glucose meter values in to corresponding 
 *                  APDU locations	
 *
 * Note:            
 *
...
void PhdGlucoseMeterUpdateMeasurements(void)
{
    ...
    BloodGlucose.Val = GetBloodGlucose();
    PhdAppBuffer[34]= BloodGlucose.byte.HB;  	//Measured analog voltage LSB
    PhdAppBuffer[35]= BloodGlucose.byte.LB;  	//Measured analog voltage MSB
...
/******************************************************************************
 * Function:        UINT8 GetBloodGlucose(void)
 *
 * PreCondition:    None
 *
 * Input:           None
 ...
 * Side Effects:    None
 *
 * Overview:        This routine Gets the blood Glucose Level simulated by Potentiometer.  	
 *
 * Note:            
 *
 *****************************************************************************/
UINT16 GetBloodGlucose(void)
{
  UINT16 bloodGlucose; 
