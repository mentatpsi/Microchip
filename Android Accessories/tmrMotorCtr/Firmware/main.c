...
/**/
   ...
   // _CONFIG3( WPCFG_WPCFGDIS & WPDIS_WPDIS)		//Disable erase/write protect of all memory regions.
        _CONFIG3(
                ...
                WPDIS_WPDIS );//&		//Disable erase/write protect of all memory regions.
...
int rpm2Setting(int curRate) {
    int per = (curRate / 12499) * 100;
    return toSetting(per);
}
...
void per2LEDs(int percentage) {
    int setting = toSetting(percentage);
    SetLEDs(setting);
}

  void pot2LEDs() {
      int setting;
      int percentage = ReadPOT();




        if (percentage < 10)     { setting = 0b00000000;}
        if (percentage >= 10)    { setting = 0b10000000;}
        if (percentage >= 50)    { setting = 0b11000000;}
        if (percentage >= 75)    { setting = 0b11100000;}
        if (percentage == 100)   { setting = 0b11110000;}
      /*
       *

      switch (percentage) {
          case 0:     { setting = 0b00000000;break;}
          case 10:    { setting = 0b10000000;break;}
          case 25:    { setting = 0b11000000;break;}
          case 50:    { setting = 0b11100000;break;}
          case 75:    { setting = 0b11110000;break;}
          case 100:   { setting = 0b11111000;break;}

      }


       *
      if (percentage == 0x64)   { setting = 0b11111000;}
      if (percentage >= 0x4B)    { setting = 0b11110000;}
      if (percentage >= 0x32)    { setting = 0b11100000;}
      if (percentage >= 0x19)    { setting = 0b11000000;}
      if (percentage >= 0x10)    { setting = 0b10000000;}
      if (percentage == 0x0)     { setting = 0b00000000;}
      */
  ...
  void rpm2LEDs(DWORD curRate) {
      ...
      int setting = rpm2Setting(curRate);
...
int main(void)
{
    ...
    while(1)
    {
        ...
        {
            if (pwmMode == 1) {  LED2_On(); LED1_Off();}
            if (pwmMode == 2) {  LED2_Off(); LED1_On();}
            ...
            if ((curPush == 0x8) || (mode == 1)) {
                ...
                if(tempValue != potPercentage) {
                    ...
                    if (pwmMode == 1) {
                        SetPWMPer(tempValue,0);
                    }
                    if (pwmMode == 2) {
                        SetPWMPer(tempValue,1);
                        //fourWave();
                    }
                    pot2LEDs();
                    //setRPM(tempValue);
                    //pot2LEDs();
                   // SetDutyCycle_PWM(tempValue);
                    potPercentage = tempValue;
                }
            }
            ...
            else
            {
                switch(command_packet->command)
                {
                    ...
                    case COMMAND_SET_PWM:
                        //int percent = command_packet->data;
                        per2LEDs(command_packet->data);
                        valueNeedsUpdate = TRUE;
                        BYTE cur = command_packet->data;
        ...
        if((valueNeedsUpdate == TRUE) && (writeInProgress == FALSE))
        {
            outgoing_packet.command = COMMAND_UPDATE_POT;
            outgoing_packet.data = writeValue;
